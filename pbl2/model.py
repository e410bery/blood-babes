...
km = c.Km_maoA
ki = c.Ki_maoi_maoA
inhibitor = inhibitor_time(time)
sub0 = c.Serotonin_conc_B
P0 = 0
initial_conditions = [P0, sub0]
t = np.linspace(0, 50, 51) 

def solver(tcurrent,y):
    P, sub = y
    dP_dt = (vmax * sub) / (km * (1 + inhibitor[int(tcurrent)] / ki) + sub)
    dsub_dt = -((vmax*sub) / (km * (1 + inhibitor[int(tcurrent)] / ki) + sub))
    return[dP_dt, dsub_dt]

solution = solve_ivp(solver, [t[0], t[-1]], initial_conditions, t_eval=t, method='BDF')
...
plt.savefig("pbl2/graphs/model_output.png")
